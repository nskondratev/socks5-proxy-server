...
redis.connect()
  .then(() => {
    ...
    server.on('authenticate', async username => {
      ...
      await redis.hSet(CONSTANTS.REDIS.AUTH_DATE_KEY, username, (new Date()).toISOString())
    })
    ...
    server.on('proxyData', async ({ data, user, kill }) => {
      try {
        if (user) {
          ...
          await redis.hIncrBy(CONSTANTS.REDIS.DATA_USAGE_KEY, user.username, data.length)
        }
      } catch (err) {
        logger.error(err)
      }
    ...
    schedule.scheduleJob('0 0 0 1 * *', async () => {
      ...
      try {
        await redis.del(CONSTANTS.REDIS.DATA_USAGE_KEY)
